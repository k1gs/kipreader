...
from .utils import read_bytes_from_file, load_kips_by_class, edit_bytes_in_file, copy_file_to_switch, upload_file_via_ftp, CUST_OFFSET, load_material_options
...
class ByteEditor(QWidget):
    ...
    def open_file(self):
        fname, _ = QFileDialog.getOpenFileName(self, "Выберите .kip", "", "All Files (*)")
        if fname:
            self.filename = fname
            param_values = []
            with open("kips.json", "r", encoding="utf-8") as f:
            ...
            for param in all_params:
                ...
                if options and len(options) > 0:
                    length = len(bytes.fromhex(options[0][1]))
                else:
                    length = 1
                value_bytes = read_bytes_from_file(self.filename, offset, length)
                value_hex = value_bytes.hex()
                param_name = param.get("name_of_param", "Параметр")
                param_group = param.get("class", "Прочее")
                param_values.append((param_name, value_hex, param_group))
            dlg = ParamsDialog(param_values, self)
            dlg.exec_()
    def edit_bytes(self):
